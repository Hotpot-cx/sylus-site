...
if (cardGridEl) {
  ...
  cardGridEl.addEventListener("mousemove", (e) => {
    ...
    if (
      ...
      (isSwiping ||
        stackSpringLock ||
        document.getElementById("cardModal").classList.contains("active"))
    )
      return;
    const cards = isStack
    ...
    cards.forEach((card) => {
      ...
      if (distance < maxDistance) {
        ...
        const phase =
          scrollOffset * 0.3 +
          Array.from(cardGridEl.children).indexOf(card) * 0.5;
...
if (cardGridForView) {
  ...
  // Touch support
  cardGridForView.addEventListener(
    "touchstart",
    (e) => {
      if (currentView !== "stack") return;
      isSwiping = true;
...
const stackHintHTML =
  '<div class="stack-hint"><span class="stack-hint-item"><span class="stack-hint-arrow">◀</span> 左滑看下一张</span><span class="stack-hint-item">右滑看详情 <span class="stack-hint-arrow">▶</span></span></div>';
const stackProgressHTML =
...
function openModal(card) {
  ...
  for (let i = 1; i <= 5; i++) {
    if (i <= rarity) {
      starsHtml +=
        '<svg class="modal-star" viewBox="0 0 24 24"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>';
    } else {
      starsHtml +=
        '<svg class="modal-star empty" viewBox="0 0 24 24"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>';
    }
  }
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Enter" && !cardModal.classList.contains("active")) {
    ...
    if (
      focused &&
      focused.classList.contains("card") &&
      focused.dataset.cardName
    ) {
      openModal(focused);
    }
  }
...
const dialogueMap = {
  home: "嘎！欢迎来到 N109 区！我是梅菲斯特～新混池「一幕一生」6.17开启，老大的「别幕终曲」来了！",
  ...
  schedule: "嘎嘎！新混池「一幕一生」6.17开启！五选三自选，老大的「别幕终曲」必选！",
...
function animateMascot() {
  if (mascotVisible && !document.hidden) {
    ...
    const clampedX = Math.min(
      Math.max(currentX, marginX),
      window.innerWidth - marginX,
    );
    const clampedY = Math.min(
      Math.max(currentY, marginTop),
...
const playlist = [
  {
    name: "有形之锢",
    artist: "恋与深空 OST",
    src: "music/恋与深空-有形之锢-_mqms2_.mp3",
  },
  {
    name: "交错视界",
    artist: "Mikelangelo Loconte · 2.0 主题曲",
    src: "music/恋与深空-交错视界-_mqms2_.mp3",
  },
  {
    name: "不设防禁区",
    artist: "恋与深空 OST",
    src: "music/恋与深空-不设防禁区-_mqms2_.mp3",
  },
  {
    name: "炽光淋漓",
    artist: "恋与深空 OST",
    src: "music/恋与深空-炽光淋漓-_mqms2_.mp3",
  },
  {
    name: "即兴放逐",
    artist: "恋与深空 OST",
    src: "music/恋与深空-即兴放逐-_mqms2_.mp3",
  },
  {
    name: "久候狂欢之徒",
    artist: "恋与深空 OST",
    src: "music/恋与深空-久候狂欢之徒-_mqms2_.mp3",
  },
  {
    name: "龙谷咏叹",
    artist: "恋与深空 OST",
    src: "music/恋与深空-龙谷咏叹-_mqms2_.mp3",
  },
  {
    name: "无桎绿野",
    artist: "恋与深空 OST",
    src: "music/恋与深空-无桎绿野-_mqms2_.mp3",
  },
  {
    name: "银翼安魂地",
    artist: "恋与深空 OST",
    src: "music/恋与深空-银翼安魂地-_mqms2_.mp3",
  },
];
...
document.addEventListener("click", (e) => {
  if (
    !bgmPanel.contains(e.target) &&
    e.target !== bgmBtn &&
    !bgmBtn.contains(e.target)
  ) {
    bgmPanel.classList.remove("open");
  }
});
